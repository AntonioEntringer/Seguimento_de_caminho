...
### Animação/GIF
O seguimento do círculo é representado graficamente, com destaque para a orientação do drone simulado (eixos: Vermelho para X, Verde para Y e Azul para Z).  
...
Foram calculadas três retas com pontos que preenchem o caminho entre elas, sem restrição de tempo.
...
**Erro de velocidade:**  
Durante o experimento, trafeguei o drone sobre o caminho para frente e para trás, o que ocasionou variações significativas de velocidade.  
...
Semelhante ao exemplo anterior de retas concatenadas, desta vez a velocidade não é definida pelo joystick (multiplicando a norma da direção desejada), mas sim pela definição de tempo passada no momento da geração do caminho, onde cada segmento carrega consigo a velocidade desejada. Mantive a característica de controlar com o joystick para trafegar em sentido inverso, mas a velocidade deixa de ser um vetor unitário.
...
Como não há restrição, a velocidade assume o maior valor logo em tempo igual a zero.  
...
O seguimento não é suave, pois a velocidade é constante, ocorrendo um degrau no início e, por consequência, uma aceleração infinita (limitada pelos 10°), gerando uma descontinuidade na aceleração.
...
**Erro de velocidade:**  
É possível observar o degrau quando solicito a trajetória ao contrário no momento final. Nos casos mais complexos, a velocidade final é zero graças às constraints requisitadas.
...
Neste caso, uma matriz contendo as posições e restrições é passada para gerar o caminho, que possui requisição de posição, velocidade e aceleração. Por limitação da ordem da matriz, não é possível solicitar variação suave da aceleração, e o drone realiza mudanças de inclinação abruptas.
...
A posição e velocidade são suaves ao longo do tempo, mas há descontinuidade no jerk, o que faz a aceleração variar abruptamente.
...
**Erro de orientação:**  
Observa-se que o drone satura os 100° até aproximar-se da orientação desejada. Em um modelo real, ganhos de orientação muito altos podem ser indesejáveis por demandarem muita energia.  
...
Aqui, a matriz de posições e restrições é de ordem superior, permitindo continuidade na variação da aceleração, que se torna suave.
...
### Animação/GIF – Seguimento pelo Drone
Observa-se que o início e o fim da trajetória ocorrem de forma suave, com a maior velocidade próxima ao centro.  
...
Assim como no caso dos 2 waypoints, a velocidade é definida pelo valor necessário para ir de um ponto ao outro, resultando em descontinuidade na aceleração.
...
### Animação/GIF – Seguimento pelo Drone
Devido à velocidade constante, há um degrau de velocidade que implica uma aceleração infinita (limitada pelos 10°).
...
**Erro de velocidade:**  
É possível observar o degrau quando a trajetória é invertida. Em casos mais complexos, a velocidade final é zero graças às constraints.
...
Neste caso, uma matriz contendo as posições e restrições é utilizada para gerar o caminho, com requisição de posição, velocidade e aceleração. Devido à ordem da matriz, não é possível requisitar variação suave da aceleração, e o drone realiza mudanças de inclinação abruptas.
...
A posição e a velocidade são suaves ao longo do tempo, mas há descontinuidade no jerk, resultando em variação abrupta da aceleração entre os waypoints.
...
### Resultado – Erro de Posição, Velocidade e Orientação
**Erro de posição:**  
<img src="4waypoints_3ordem/Posicoes-4waypoints_4ordem.txt.png" alt="Erro de Posição - 4 Waypoints Ordem Cúbica" width="600" />

**Erro de velocidade:**  
<img src="4waypoints_3ordem/Velocidades-4waypoints_4ordem.txt.png" alt="Erro de Velocidade - 4 Waypoints Ordem Cúbica" width="600" />

**Erro de orientação:**  
Com polinômio de ordem cúbica não é possível obter variação constante na aceleração; o jerk apresenta descontinuidade, a qual é corrigida com polinômios de ordem superior e constraints apropriadas.
<img src="4waypoints_3ordem/Orientacoes-4waypoints_4ordem.txt.png" alt="Erro de Orientação - 4 Waypoints Ordem Cúbica" width="600" />
...
*Observação:*  
Substitua os textos entre parênteses com explicações detalhadas conforme sua necessidade. Certifique-se de que os nomes dos arquivos de imagem e GIF estejam corretos e correspondam aos nomes reais presentes em cada pasta.
